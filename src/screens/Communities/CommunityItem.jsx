...
const CommunityItem = ({ community, onUpdate, onViewCommunity }) => {
  ...
  const { createdAt } = community; // Moment(community.createdAt).format("MMM d, HH:mm");
  ...
  const onLeaveCommunity = async () => {
    ...
    try {
      ...
      // TODO  a proper way to refresh feed without blinking
      onUpdate();
    ...
    <Surface style={styles.container}>
      <View style={styles.feedItem}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.post}>Is Public: {community.isPublic ? 'Yes' : 'No'}</Text>
          ...
          >
            ...
            /> */}
              {loading ? (
                ...
                <Text style={{ marginStart: 10 }}>{community.isJoined ? 'Leave' : 'Join'}</Text>
              )}
            </Pressable>
            <Pressable
              style={{
